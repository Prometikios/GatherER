...
def ASKforINPUT():
    ...
    for i, eachItem in enumerate(WordsStorage):
        if ' ' in eachItem:
            ...
            WordsStorage[i] = eachItem, URLbase + 'video/' + eachItem.replace(' ', '-') + '/'
        else:
            #WordsStorage[i] = eachItem, URLbase + eachItem + "-tarifi/"
            WordsStorage[i] = eachItem, URLbase + 'video/' + eachItem + "/"
...
def DinnerDictMaker(driver, nameOFdinner, InputURL):
    ...
    print('done waiting?')
    EachItem = driver.find_elements(By.CSS_SELECTOR, 'li[itemprop="recipeIngredient"]')
